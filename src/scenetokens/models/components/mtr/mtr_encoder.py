...
from .mtr_utils import PointNetPolylineEncoder
...
class MTREncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Transformer self-attention layers (global only; local requires CUDA KNN)
        self.use_local_attn = self.model_cfg.get('use_local_attn', False)
        if self.use_local_attn:
            raise ValueError(
                "use_local_attn=True in the encoder requires the custom CUDA KNN extension "
                "from the original MTR repository. Set use_local_attn=False to use global "
                "attention, which does not require CUDA compilation."
            )
